...
FVector UGravityMovementComponent::GetGravityDir(bool bAvoidZeroGravity) const
{
...
void UGravityMovementComponent::SetGravityDir(const FVector& NewGravityDirection)
{
...
void UGravityMovementComponent::PhysFlying(float deltaTime, int32 Iterations)
{
	...
	const FVector GravityDir = GetGravityDir();
...
float UGravityMovementComponent::BoostAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration)
{
	...
	if (AirControlBoostMultiplier > 0.0f && FVector::VectorPlaneProject(Velocity, GetGravityDir(true)).SizeSquared() < FMath::Square(AirControlBoostVelocityThreshold))
	{
...
FVector UGravityMovementComponent::CalcAnimRootMotionVelocity(const FVector& RootMotionDeltaMove, float DeltaTime, const FVector& CurrentVelocity) const
{
	...
	if (IsFalling())
	{
		const FVector GravityDir = GetGravityDir(true);
...
void UGravityMovementComponent::FindFloor(const FVector& CapsuleLocation, FFindFloorResult& OutFloorResult, bool bZeroDelta, const FHitResult* DownwardSweepResult /*= NULL*/) const
{
	...
	if (FloorLineTraceDist > 0.f || FloorSweepTraceDist > 0.f)
	{
		...
		else
		{
			...
			// TODO 5.4
			const bool IsActorBasePendingKill = BaseActor && BaseActor->IsPendingKillPending();
...
{
	return GetGravityDir(true) * -1.0f;
}
